...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul  5 20:09:33 2021
...
def makeScatterPlot(comparisonOutcome):
	...
	for arr in comparisonOutcome[1:]:
		...
		for i in range(16,22):
			if arr[i-14] == "TIMEDOUT":
				continue
			else:
				if minTime > arr[i]:
					minTime = arr[i]
					minType = comparisonOutcome[0][i-14]
		...
		for i in range(1,8):
			if 'NOK' in arr[i]:
				colNow = 'g'
				break
			elif 'OK' in arr[i] or 'ReachedBound' in arr[i]:
				colNow = 'r'
				break
	...
	ax.set_ylabel('Time Taken by min(alpha10,30,50,70,90,UW)(seconds)')
...
def makeVarianceScatterPlot(runOutcome, files):
	minFolder = {}
	maxFolder = {}
	for folder in folderList:
		minTime = {}
	...
	for folder in folderList:
		...
		colOutcome = []
		for file in files:
			type1ExecTimes.append(maxFolder[folder][file])
			type2ExecTimes.append(minFolder[folder][file])
			colNow = 'b'
			if maxFolder[folder][file] == minFolder[folder][file]:
				colNow = 'b'
			elif maxFolder[folder][file] > minFolder[folder][file]:
				colNow = 'r'
			else:
				colNow = 'g'
			colOutcome.append(colNow)
		...
		#plt.axis('scaled')
		ax.scatter(type1ExecTimes, type2ExecTimes, color=colOutcome)
		ax.set_xlabel('min Time Taken by '+folder+'(seconds)')
		ax.set_ylabel('max Time Taken by '+folder+'(seconds)')
		line = mlines.Line2D([0, 1], [0, 1], color='red')
		transform = ax.transAxes
		line.set_transform(transform)
		ax.add_line(line)
		...
		with open(myDir + folder + '/' + run + '_Result.csv') as csv_file:
			...
			for row in csv_reader:
				if line_count == 0:
					...
					# Tuple for every file = {'NAME', 'Outcome', (float)RunTime, (int)TotalProeduresInlined}
