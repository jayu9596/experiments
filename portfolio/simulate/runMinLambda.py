...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 22 18:11:50 2021
...
import pandas as pd
myDir = '/home/jaydeep/Thesis/experiments/portfolio/simulate/'
uwFolder = '3clients/'
uwFolderHeader = 'min(AlphaDecay)'
uwOptions = ['lambda0.01','lambda0.005','lambda0.025','lambda0.055']
vanillaFolder = 'OR'
maxValue = 3600
...
# UW
for option in uwOptions:
    uwOptionFolder = uwFolder + option
    f = open(myDir + uwOptionFolder + 'Result.csv','w+')
    f.write("Name,OutCome,RunTime,TotalSplits,BoogieDumpTime\n")
    i = 0
    for root, dirs, files in os.walk(myDir + uwOptionFolder + '/'):
        for file in files:
            if file.endswith(".bpl.txt"):
                i+=1
    #            print(str(i) + '\n')
...
uwFilesOutcome = {}
for option in uwOptions:
    uwFilesOutcome[option] = {}
for option in uwOptions:
    uwOptionFolder = uwFolder + option
    uwOptionOutcome = {}
    with open(myDir + uwOptionFolder + 'Result.csv') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        line_count = 0
        for row in csv_reader:
            # print(row)
            if line_count == 0:
                ...
                boogieDumpTime = float(row[4])
                totalTime = float(row[2]) - boogieDumpTime
                uwOptionOutcome[str(row[0])] = (str(row[0]),str(row[1]),totalTime,int(row[3]))
                line_count += 1
    uwFilesOutcome[option] = uwOptionOutcome

def getBestUW(uwData, file):
    mnTime = 9999999
    bestUW = 'None'
    for option in uwOptions:
        if mnTime > uwData[option][file][2]:
            bestUW = option
            mnTime = uwData[option][file][2]
    return bestUW
...
colFiltered = []
comparisonOutcome.append(['Name',vanillaFolder + '_OutCome',uwFolderHeader + '_Outcome',vanillaFolder + '_TotalSplits',uwFolderHeader + '_TotalSplits',vanillaFolder + '_RunTime',uwFolderHeader + '_RunTime','Who Performed Better','By How Much Percentage',uwFolderHeader + ' TimedOut but not ' + vanillaFolder,vanillaFolder + ' TimedOut but not ' + uwFolderHeader])
...
#    if file not in uwFilesOutcome or file not in vanillaFilesOutcome:
            ...
            #print(file)
#        continue
    uwBestOptoion = getBestUW(uwFilesOutcome, file)
    uwData = uwFilesOutcome[uwBestOptoion][file]
...
ax.set_ylabel('Time Taken by '+uwFolderHeader+'(seconds)')
